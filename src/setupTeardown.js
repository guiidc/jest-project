...
const randomAttack = () => {
  const randomMonster = enemies[Math.floor(Math.random() * 4)]; // obj
  const randomSpecialistIndex = Math.floor(Math.random() * specialists.length); // number
  const randomSpecialist = specialists[randomSpecialistIndex]; // obj
